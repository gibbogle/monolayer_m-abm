...
!--------------------------------------------------------------------------
subroutine get_metab_rates(ityp, H, Cin, G_rate, PP_rate, PO_rate)
integer :: ityp
real(REAL_KIND) :: H, Cin(:)
...
do ityp = 1,Ncelltypes
	M => metabolic(ityp)
	call get_metab_rates(ityp, M%HIF1, Caverage(1:MAX_CHEMO), M%G_rate, M%PP_rate, M%PO_rate)
